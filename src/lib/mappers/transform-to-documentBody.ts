import type { GRTESimpleDocumentBody } from "@/types/GRTE-data";
 ...
 */
export const transformToGRTEDocumentBody = (data: GRTESimpleDocumentBody): GRTEDocumentBody => {
  ...
  return {
    ...
    "cac:Shipment": {
      ...
      "cac:ShipmentStage": {
        ...
        "cac:DriverPerson": data.conductores.map(conductor => ({
          "cbc:ID": {
            "_attributes": { "schemeID": conductor.tipoDocumento },
            "_text": conductor.numeroDocumento,
          },
          "cbc:FirstName": { "_text": conductor.nombres },
          "cbc:FamilyName": { "_text": conductor.apellidos },
          "cbc:JobTitle": { "_text": "Principal" }, // Valor fijo o desde data
          "cac:IdentityDocumentReference": {
            "cbc:ID": { "_text": conductor.numeroLicencia },
          },
        })),
      },
      "cac:Delivery": {
    ...
    // 5. Líneas de Ítems
    "cac:DespatchLine": data.items.map(item => ({
      "cbc:ID": { "_text": item.numero },
      "cbc:DeliveredQuantity": {
        "_attributes": { "unitCode": item.unidadMedida },
        "_text": item.cantidad,
      },
      "cac:OrderLineReference": {
        "cbc:LineID": { "_text": item.numero },
      },
      "cac:Item": {
